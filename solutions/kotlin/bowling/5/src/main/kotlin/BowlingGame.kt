class BowlingGame {
    ...
    fun roll(pins: Int) {
       ...
       if (pins < 0 || pins > 10) {
          throw IllegalStateException("Invalid pin count: $pins")
     }
        ...
        if (framesSoFar < 9) {
            ...
            if (tenthStart >= 0 && index >= tenthStart + 2) {
                ...
                if (firstRoll == 10) {
                     val bonus1 = rolls.getOrNull(tenthStart + 1)
                     if (bonus1 != null && bonus1 < 10 && index == tenthStart + 2 && bonus1 + pins > 10) {
                     throw IllegalStateException("Two bonus rolls after strike cannot score more than 10 pins total unless first is strike")
              }
                } else {
                    // firstRoll < 10
